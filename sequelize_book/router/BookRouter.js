...
async function updateBook(req,res){
    try {
        ...
        const info = await Books.putUpdateBook(BookIndex, title, writer, year, summary,history);

        res.render('succesUpdate', {list:"책 목록",view : info} );
...
async function showBookDetail(req, res) {
    try {
        ...
        console.log('BookIndex : ', BookIndex);
        const info = await Books.getBookDetail(BookIndex);
        res.render('detailBook', {title:"책 목록",  info: info, v_info:info.bookinfo.datavalues} );
    }
    catch ( error ) {
        console.log('Can not find, 404');
        res.status(error.code).send({msg:error.msg});
...
async function addBook(req, res) {
    ...
    try {
        const result = await Books.addBook(title, writer, year, summary,history);
        res.render('succesUpdate', {list:"책 목록",view : result} );
    }
    catch ( error ) {
